    ...
    <div className="bg-white/5 rounded-xl p-4 shadow border border-white/10">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-3 py-2 rounded-[12px] bg-[hsl(var(--gold))] text-[hsl(var(--royal))] font-semibold shadow hover:shadow-[0_0_16px_rgba(255,215,0,.45)] flex items-center gap-2"
        >
              ...
              >
                <div>
                  ...
                  <div className="text-xs opacity-80">
                    {t.name} · {t.count} ppl · ₹{t.amount.toLocaleString()}
                  </div>
                </div>
      ...
      <TicketPurchase
        ...
        onSuccess={(t) => {
          ...
          toast.success("E‑Darshan ticket booked", {
            description: `${t.date} ${t.slot} · ${t.count} devotees · ₹${t.amount.toLocaleString()}`,
          });
        }}
      />
    </div>
...
export default function Dashboard() {
  ...
  const basePrediction = useMemo(
    () =>
      Array.from({ length: 24 }).map((_, h) => {
        const base =
          2000 +
          8000 * Math.sin((Math.PI * (h + 6)) / 24) ** 2 +
          (h > 18 && h < 22 ? 2000 : 0);
    ...
    const id = setInterval(() => {
      setParking((ps) =>
        ps.map((p) => ({
          ...
          used: Math.min(
            p.total,
            Math.max(0, p.used + Math.round((Math.random() - 0.5) * 20)),
          ),
        })),
      );
    }, 7000);
  ...
  const occColor =
    occupancy < 60
      ? "text-[hsl(var(--green-safe))]"
      : occupancy <= 80
        ? "text-amber-400"
        : "text-[hsl(var(--red-alert))]";
      ...
      <div className="mx-auto max-w-[1440px] px-6 md:px-10 lg:px-20 py-8 grid grid-cols-10 gap-6">
        ...
        <div className="col-span-10 md:col-span-3 space-y-6">
          <div className="bg-white/5 rounded-xl p-5 shadow border border-white/10">
            ...
            <div className="mt-1 text-sm opacity-80">
              Occupancy: <span className={occColor}>{occupancy}%</span>
            </div>
          ...
          <div className="bg-white/5 rounded-xl p-5 shadow border border-white/10">
            ...
            <div className="text-sm opacity-90">
              ...
              <span className="font-semibold">
                {queueLen.toLocaleString()} people
              </span>
            </div>
            <div className="text-sm opacity-90">
              Avg wait: <span className="font-semibold">{avgWait} min</span>
            </div>
            <div className="mt-4">
              <Progress
                value={Math.min(100, Math.round((queueLen / 2000) * 100))}
                ...
                <BarChart data={prediction}>
                  ...
                  <ReTooltip
                    formatter={(v: any, _n: any, p: any) => [
                      v,
                      `Hour: ${String(p.payload.hour).padStart(2, "0")}:00 – Predicted Visitors`,
                    ]}
                    contentStyle={{
                      background: "#0A1D37",
                      border: "1px solid rgba(255,255,255,0.15)",
                  ...
                  <Bar dataKey="value" fill="#FFD700">
                    {prediction.map((entry, index) => (
                      <Cell
                        key={`c-${index}`}
                        fill={entry.value > 10000 ? "#FF4D4D" : "#FFD700"}
                      />
                    ))}
                  </Bar>
                </BarChart>
        ...
        <div className="col-span-10 md:col-span-3 space-y-4">
          ...
          <div className="bg-white rounded-xl p-4 shadow max-h-[280px] overflow-auto">
            ...
            <ul className="space-y-3">
              {alerts.map((a, i) => (
                <li
                  key={i}
                  className="bg-slate-50 rounded-lg p-3 shadow-sm border border-slate-200 animate-[slide-in_0.3s_ease]"
                >
          ...
          <div className="space-y-3">
            <button className="w-full px-4 py-3 rounded-[12px] bg-[hsl(var(--red-alert))] text-white font-semibold shadow hover:shadow-[0_0_18px_rgba(255,77,77,.6)] transition">
              Dispatch Medical
            </button>
            <button className="w-full px-4 py-3 rounded-[12px] bg-[hsl(var(--royal))] text-white font-semibold shadow hover:shadow-[0_0_18px_rgba(0,74,173,.6)] transition">
              Dispatch Police
            </button>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-1 text-sm">
                        Available:{" "}
                        <span className={color}>{p.total - p.used}</span> /{" "}
                        {p.total}
                      </div>
                    </div>
                  );
              ...
              <div className="relative h-40 bg-slate-100 rounded-lg overflow-hidden border border-slate-200">
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="absolute left-0 top-[20%] h-1 w-full">
                    <div
                      className="animate-[flow_5s_linear_infinite] h-1 w-24 bg-[hsl(var(--royal))] shadow-[0_0_10px_rgba(0,74,173,.4)]"
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={`b-${i}`}
                    className="absolute right-0 bottom-[20%] h-1 w-full"
                  >
                    <div
                      className="animate-[flowR_6s_linear_infinite] h-1 w-24 bg-[hsl(var(--red-alert))] shadow-[0_0_10px_rgba(255,77,77,.4)]"
